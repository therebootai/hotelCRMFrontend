        ...
        <div className="divide-y divide-gray-50">
          {loading ? (
            <div className="p-12 text-center">
              ...
              <p className="mt-2 text-xs text-gray-400 font-bold">Loading billing history...</p>
            </div>
          ) : billings.length === 0 ? (
            ...
            billings.map((bill) => {
              ...
              return (
                <div key={bill._id} className="flex items-center px-6 py-3.5 hover:bg-gray-50/50 transition-all">
                  ...
                  {/* Invoice Code */}
                  <div className="w-32">
                    <span className="font-bold text-xs text-orange-600 block">{bill.invoiceNumber}</span>
                  ...
                  <div className="w-36 text-center">
                    <span className="font-bold text-gray-800 text-xs">₹{bill.grandTotal.toLocaleString()}</span>
                  </div>

                  {/* Paid Amount */}
                  <div className="w-36 text-center">
                    <span className="font-bold text-green-600 text-xs">₹{bill.paidAmount.toLocaleString()}</span>
                  </div>

                  {/* Remaining Due / Baki */}
                  <div className="w-36 text-center">
                    <span className={`font-bold text-xs ${bill.dueAmount > 0 ? "text-red-500" : "text-gray-400"}`}>
        ...
        <div className="flex justify-between items-center bg-white px-6 py-4 rounded-xl border border-gray-100 shadow-sm">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-3 py-1.5 border border-gray-200 rounded-lg text-xs font-bold text-gray-600 disabled:opacity-40"
            >
              Previous
            </button>
            <button
