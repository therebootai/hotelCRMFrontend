...
const ProcessBillingModal: React.FC<ProcessBillingModalProps> = ({
  ...
  const filteredStays = activeCheckIns.filter((ci) => {
    ...
    const rooms = ci.roomDetails?.map((r: any) => r.roomNumber).join(", ") || "";
  ...
  const handleCheckInSelect = async (checkIn: any) => {
    ...
    const primary = checkIn.guests?.find((g: any) => g.isPrimary) || checkIn.guests?.[0];
    const rooms = checkIn.roomDetails?.map((r: any) => r.roomNumber).join(", ");
    setSearchQuery(`${checkIn.checkInId} - ${primary?.name || "Guest"} (Rooms: ${rooms})`);
    setShowDropdown(false);
    ...
    try {
      ...
      if (res.data && res.data.success) {
        ...
        const mappedServices = (preview.extraServices || []).map((s: any) => {
          const match = dbExtraServices.find((ds) => ds.name.toLowerCase() === s.serviceName.toLowerCase());
          return {
      ...
      console.error(err);
      toast.error(err.response?.data?.message || "Failed to load check-in ledger details");
      setBillPreview(null);
    } finally {
  ...
  const handleSaveCustomServiceToDb = async (index: number, customName: string) => {
    if (!customName.trim()) {
    ...
    try {
      const res = await api.post("/extra-services", { name: customName.trim(), isActive: true });
      if (res.data && res.data.success) {
      ...
      console.error(err);
      toast.error(err.response?.data?.message || "Failed to register custom service in database");
    }
  };

  // Dynamic calculations
  const roomChargesTotal = billPreview?.totalRoomCharges || 0;
  const extraServicesTotal = extraServices.reduce((sum, s) => sum + (Number(s.total) || 0), 0);
  const subTotalAmount = Math.round(roomChargesTotal + extraServicesTotal + Number(restaurantCharges || 0));
  const calculatedTax = Math.round((subTotalAmount * Number(taxPercentage || 0)) / 100);
  const calculatedGrandTotal = Math.round(subTotalAmount + calculatedTax - Number(discount || 0));
  
  // Advance payment deduction display logic
  const advanceAmount = billPreview?.advanceDeducted || 0;
  
  const netPayableAmount = Math.round(Math.max(0, calculatedGrandTotal - advanceAmount));
  const previouslyPaid = billPreview?.paidAmount || 0;

  // Total paid after this transaction
  const finalPaidAmount = Math.round(previouslyPaid + Number(paymentAmount || 0));
  const remainingDue = Math.round(Math.max(0, netPayableAmount - finalPaidAmount));
  ...
  const handleSubmitBilling = async (isCheckoutFlag: boolean) => {
    ...
    if (isCheckoutFlag && remainingDue > 0) {
      toast.error(`Cannot complete checkout. Outstanding due of ₹${remainingDue} must be fully paid.`);
      return;
    }

    if (paymentAmount > 0 && paymentMethod !== "Cash" && !paymentTxnId.trim()) {
      toast.error(`Please provide a Transaction ID for ${paymentMethod} payments.`);
      return;
    }
    ...
    const payload = {
      ...
      payment: paymentAmount > 0 ? {
        ...
        transactionId: paymentMethod !== "Cash" ? paymentTxnId.trim() : undefined,
        note: paymentNote || (isCheckoutFlag ? "Checkout settlement" : "Partial payment")
      } : undefined
    };
      ...
      <div className="bg-white rounded-2xl max-w-5xl w-full border border-gray-100 shadow-2xl flex flex-col max-h-[95vh] overflow-hidden animate-in zoom-in duration-250">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="relative" ref={dropdownRef}>
            <label className="text-[10px] uppercase font-black text-gray-400 tracking-wider block mb-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-10 py-3 bg-gray-50 border border-gray-100 focus:border-orange-300 focus:bg-white focus:ring-1 focus:ring-orange-100 rounded-xl outline-none text-xs font-bold text-gray-700 shadow-sm transition-all"
                value={searchQuery}
                onChange={(e) => {
                  ...
                  filteredStays.map((ci) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 hover:bg-orange-50/40 transition-all text-xs font-bold flex justify-between items-center ${
                          isSelected ? "bg-orange-50/20 text-orange-700" : "text-gray-700"
                        }`}
                      >
          ...
          {previewLoading && (
            <div className="py-24 text-center">
              ...
              <p className="mt-2 text-xs text-gray-400 font-bold">Recalculating stays & pulling active ledger reports...</p>
            </div>
          )}
          ...
          {!previewLoading && billPreview && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                {/* Stay Summary Info */}
                <div className="bg-orange-50/30 p-4 rounded-xl border border-orange-100/50 grid grid-cols-2 gap-4 text-xs font-bold text-gray-700">
                  <div>
                    <span className="text-[9px] uppercase text-gray-400 font-black tracking-wider block mb-0.5">Primary Guest</span>
                    <p className="text-gray-900 font-black">
                      {billPreview.primaryGuest?.name || billPreview.customerId?.name || "N/A"}
                    </p>
                    {(billPreview.primaryGuest?.mobileNo || billPreview.customerId?.phone) && (
                      <p className="text-[10px] text-gray-400 font-medium mt-0.5">
                        Mobile: {billPreview.primaryGuest?.mobileNo || billPreview.customerId?.phone}
                      </p>
                    )}
                  </div>
                  <div>
                    <span className="text-[9px] uppercase text-gray-400 font-black tracking-wider block mb-0.5">Room Breakdown</span>
                    <p className="text-orange-600">
                      {billPreview.roomChargesBreakdown?.map((r: any) => `${r.roomNumber} (${r.roomType})`).join(", ") || "N/A"}
                    </p>
                  </div>
                </div>
                ...
                {advanceAmount > 0 && (
                  <div className="bg-green-50 border border-green-100 p-4 rounded-xl flex items-center justify-between text-xs font-bold">
                    ...
                    <span className="px-3.5 py-1 bg-green-600 text-white font-black rounded-lg text-xs shadow-sm">
                    ...
                    <table className="w-full text-xs text-left">
                      <thead className="bg-gray-50 text-[9px] uppercase font-black text-gray-400 tracking-wider border-b border-gray-100">
                        <tr>
                          ...
                          <th className="px-4 py-2.5 text-center">Nights Stayed</th>
                          <th className="px-4 py-2.5 text-right">Applied Rate</th>
                          <th className="px-4 py-2.5 text-right">Total Stay Charge</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-50 font-bold text-gray-700 bg-white">
                        {billPreview.roomChargesBreakdown?.map((room: any, index: number) => (
                          <tr key={index}>
                            <td className="px-4 py-3 text-orange-600">{room.roomNumber} <span className="text-[10px] text-gray-400 font-medium">({room.roomType})</span></td>
                            <td className="px-4 py-3 text-center text-gray-600">{room.nights} night(s)</td>
                            <td className="px-4 py-3 text-right text-gray-600">₹{room.ratePerNight}</td>
                            <td className="px-4 py-3 text-right text-gray-900">₹{room.totalRoomCharge}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {extraServices.map((service, index) => {
                      const isCustom = service.serviceName.trim() && !dbExtraServices.some(ds => ds.name.toLowerCase() === service.serviceName.trim().toLowerCase());

                      return (
                        <div key={index} className="bg-gray-50/50 p-3 rounded-xl border border-gray-100 space-y-2.5 shadow-sm service-search-container">
                          <div className="flex gap-2 items-center">
                            ...
                            <div className="relative flex-1">
                              <div className="relative flex items-center">
                                <input
                                  ...
                                  className="w-full pl-3 pr-20 py-2.5 bg-white border border-gray-200 rounded-xl outline-none text-xs font-bold text-gray-700 shadow-sm focus:border-orange-300 focus:ring-1 focus:ring-orange-100 transition-all"
                                  value={service.serviceName}
                                  onChange={(e) => {
                                    updateExtraServiceRow(index, "serviceName", e.target.value);
                                ...
                                <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                                  {isCustom ? (
                                    <button
                                      type="button"
                                      onClick={() => handleSaveCustomServiceToDb(index, service.serviceName)}
                                    ...
                                    <button
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        setFocusedRowIndex(focusedRowIndex === index ? null : index);
                                      }}
                              ...
                              {focusedRowIndex === index && (
                                <div className="absolute z-30 w-full mt-1.5 bg-white border border-gray-200 rounded-xl shadow-2xl max-h-48 overflow-y-auto divide-y divide-gray-50 animate-in fade-in slide-in-from-top-1 duration-200">
                                  {dbExtraServices
                                    .filter(ds => ds.name.toLowerCase().includes(service.serviceName.toLowerCase()))
                                    .map((ds) => (
                                      <button
                                        key={ds._id}
                                        type="button"
                                        ...
                                        className="w-full text-left px-4 py-2.5 hover:bg-orange-50/40 transition-all text-xs font-bold text-gray-700 flex justify-between items-center"
                                      >
                                        <span>{ds.name}</span>
                                        <span className="text-[9px] text-orange-600 font-bold font-mono uppercase bg-orange-50 px-1.5 py-0.5 rounded">Master</span>
                                      </button>
                                    ))
                                  }

                                  {/* If no exact match and user has typed something custom */}
                                  {service.serviceName.trim() && !dbExtraServices.some(ds => ds.name.toLowerCase().includes(service.serviceName.toLowerCase())) && (
                                    <div className="p-3 text-center text-[10px] text-gray-400 font-bold italic">
                                      No master service matches. Click "Add" button inside the input to save it!
                                    </div>
                                  )}
                                </div>
                              )}
                            ...
                            <input
                              ...
                              placeholder="Qty"
                              min="1"
                              className="w-16 px-3 py-2.5 bg-white border border-gray-200 rounded-xl outline-none text-xs text-center font-bold focus:border-orange-300 focus:ring-1 focus:ring-orange-100 transition-all"
                              value={service.quantity}
                              onChange={(e) => updateExtraServiceRow(index, "quantity", Number(e.target.value))}
                            />
                            
                            <input
                              type="text"
                              placeholder="Rate"
                              className="w-20 px-3 py-2.5 bg-white border border-gray-200 rounded-xl outline-none text-xs text-right font-bold focus:border-orange-300 focus:ring-1 focus:ring-orange-100 transition-all"
                              value={service.rate || ""}
                              onChange={(e) => {
                                const cleaned = e.target.value.replace(/[^0-9]/g, "");
                                updateExtraServiceRow(index, "rate", cleaned === "" ? 0 : Number(cleaned));
                              }}
                            />

                            <span className="w-20 text-right text-xs font-black text-gray-700 px-1">
                    ...
                    {extraServices.length === 0 && (
                      <p className="text-[11px] text-gray-400 font-bold italic py-4 text-center border border-dashed border-gray-100 rounded-xl">
                        No additional laundry, food, or amenities services logged yet.
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="text-[9px] uppercase font-black text-gray-400 tracking-wider block mb-1">Restaurant Charges (₹)</label>
                    <input
                      type="text"
                      className="w-full px-3 py-2.5 bg-gray-50 border border-transparent focus:border-orange-200 focus:bg-white rounded-xl outline-none text-xs font-bold"
                      value={restaurantCharges || ""}
                      onChange={(e) => {
                        const cleaned = e.target.value.replace(/[^0-9]/g, "");
                        setRestaurantCharges(cleaned === "" ? 0 : Number(cleaned));
                      }}
                    />
                  </div>
                  
                  <div>
                    <label className="text-[9px] uppercase font-black text-gray-400 tracking-wider block mb-1">Discount Amount (₹)</label>
                    <input
                      type="text"
                      className="w-full px-3 py-2.5 bg-gray-50 border border-transparent focus:border-orange-200 focus:bg-white rounded-xl outline-none text-xs font-bold text-red-500"
                      value={discount || ""}
                      onChange={(e) => {
                        const cleaned = e.target.value.replace(/[^0-9]/g, "");
                ...
                <div>
                  <label className="text-[9px] uppercase font-black text-gray-400 tracking-wider block mb-1">Billing Notes / Comments</label>
                  <textarea
                    rows={2}
                    className="w-full px-3 py-2 bg-gray-50 border border-transparent focus:border-orange-200 focus:bg-white rounded-xl outline-none text-xs font-medium"
              ...
              <div className="bg-gray-50/50 p-5 rounded-2xl border border-gray-100 space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-2 text-xs font-bold text-gray-600">
                    ...
                    <div className="flex justify-between border-t border-gray-200 pt-2 text-sm font-black text-gray-800 bg-orange-50/50 p-2 rounded-lg border border-orange-100/30">
                      <span>Grand Total:</span>
                      <span>₹{calculatedGrandTotal.toLocaleString()}</span>
                    </div>

                    <div className="flex justify-between text-sm font-black text-orange-600 bg-white p-2 rounded-lg border border-gray-100">
                ...
                <div className="border-t border-gray-200 pt-4 space-y-4">
                  ...
                  <div>
                    <label className="text-[9px] uppercase font-black text-gray-400 tracking-wider block mb-1">Payment Amount (₹)</label>
                    <input
                      type="text"
                      className="w-full px-3 py-2 bg-white border border-gray-200 focus:border-orange-200 rounded-xl outline-none text-xs font-bold text-green-600 shadow-sm animate-pulse"
                      value={paymentAmount || ""}
                      onChange={(e) => {
                        const cleaned = e.target.value.replace(/[^0-9]/g, "");
                  ...
                  <div>
                    <label className="text-[9px] uppercase font-black text-gray-400 tracking-wider block mb-1">Payment Mode</label>
                    <select
                      className="w-full px-3 py-2 bg-white border border-gray-200 rounded-xl outline-none text-xs font-bold text-gray-600 shadow-sm"
                      value={paymentMethod}
                  ...
                  {paymentMethod !== "Cash" && (
                    <div className="animate-in slide-in-from-top-1 duration-150">
                      <label className="text-[9px] uppercase font-black text-red-500 tracking-wider block mb-1 flex items-center gap-1">
                        Transaction Ref ID <span className="font-bold text-[8px] px-1 bg-red-100 text-red-600 rounded">Required</span>
                      </label>
                      <div className="relative">
                        {paymentMethod === "UPI" ? (
                        ...
                        <input
                          ...
                          className="w-full pl-9 pr-3 py-2 bg-white border border-gray-200 focus:border-red-200 focus:ring-1 focus:ring-red-100 rounded-xl outline-none text-xs font-mono font-bold"
                          value={paymentTxnId}
                  ...
                  <div>
                    <label className="text-[9px] uppercase font-black text-gray-400 tracking-wider block mb-1">Payment note</label>
                    <input
                      type="text"
                  ...
                  <div className="bg-red-50 border border-red-100 p-3.5 rounded-xl font-bold space-y-1 animate-in zoom-in-95 duration-200">
                    ...
                    <div className="flex justify-between text-xs text-red-600 border-t border-red-200/50 pt-1.5">
                      <span>Remaining Balance Due:</span>
                      <span className="font-black text-sm">₹{remainingDue.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
        ...
        <div className="border-t border-gray-100 px-6 py-4 bg-gray-50/50 flex justify-between gap-3">
          <button
            type="button"
            onClick={onClose}
            className="px-5 py-2.5 border border-gray-200 text-gray-600 rounded-xl hover:bg-gray-100 font-bold text-xs"
          >
            Cancel
          </button>
          ...
          {billPreview && (
            <div className="flex gap-2">
              <button
                ...
                className="px-5 py-2.5 bg-gray-800 text-white rounded-xl hover:bg-gray-900 font-bold text-xs active:scale-95 transition-all disabled:opacity-40"
              >
              ...
              <button
                ...
                className="px-5 py-2.5 bg-gradient-to-r from-orange-500 to-orange-400 hover:from-orange-600 hover:to-orange-500 text-white rounded-xl font-bold text-xs shadow-lg shadow-orange-100 active:scale-95 transition-all disabled:opacity-40 disabled:pointer-events-none disabled:shadow-none"
              >
