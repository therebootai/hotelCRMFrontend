...
const BookingBar: React.FC<BookingBarProps> = ({ booking, startColumn, totalNights, columnsCount }) => {
 ...
 <p className="text-[11px] font-bold text-gray-800 truncate ">{booking.guest.name || "N/A"}</p>
...
const RoomRow: React.FC<RoomRowProps> = ({ room, columns, timelineStartDate }) => {
 ...
 <div
 className="sticky left-0 z-20 bg-white flex-shrink-0 border-r border-gray-100 p-2 flex items-center gap-2 shadow-[2px_0_4px_rgba(0,0,0,0.05)] w-[140px] min-w-[140px] "
 >
 ...
 <p className="text-sm font-black text-gray-800 truncate ">Room {room.number}</p>
...
const BookingTimeline: React.FC<BookingTimelineProps> = ({ data, viewMode: _viewMode, loading }) => {
 ...
 {/* Room column header */}
 <div
 className="flex-shrink-0 sticky left-0 z-20 bg-white border-r border-gray-100 px-3 py-2 shadow-[2px_0_4px_rgba(0,0,0,0.05)] flex items-center w-[140px] min-w-[140px] "
 >
 <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wider ">Rooms</span>
 </div>
 {/* Date columns */}
 ...
 >
 <span className={`text-[10px] font-medium ${col.isToday ? "text-red-600" : "text-gray-400"}`}>
 {col.dayName}
 </span>
 <span className={`text-sm font-black ${col.isToday ? "text-red-600" : "text-gray-700"}`}>
 {col.dayNumber}
 </span>
 {col.isToday && (
...
const BookingOverview: React.FC = () => {
 ...
 <span className="text-base font-black text-gray-800 ">{stats?.totalRooms || 0}</span>
 </div>
 ...
 <span className="text-base font-black text-orange-600 ">{stats?.occupiedRooms || 0}</span>
 </div>
 ...
 <span className="text-base font-black text-green-600 ">{stats?.availableRooms || 0}</span>
 </div>
 ...
 <span className="text-base font-black text-blue-600 ">{stats?.activeBookings || 0}</span>
 </div>
