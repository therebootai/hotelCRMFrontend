...
const CheckInForm = ({
  ...
  const [showEarlyCheckInWarning, setShowEarlyCheckInWarning] =
  ...
  useEffect(() => {
    if (bookingData && !editMode && !existingCheckIn) {
      const scheduledCheckIn = bookingData.overallCheckInDate
        ? new Date(bookingData.overallCheckInDate)
        : null;
      if (scheduledCheckIn) {
        const now = new Date();
        const startOfScheduled = new Date(
          scheduledCheckIn.getFullYear(),
          scheduledCheckIn.getMonth(),
          scheduledCheckIn.getDate(),
        );
    ...
    if (editMode && existingCheckIn) {
      ...
      if (existingCheckIn.corporateCheckInDetails) {
        setCorporateDetails({
          ...
          companyAddress:
            existingCheckIn.corporateCheckInDetails.companyAddress || "",
          companyEmail:
            existingCheckIn.corporateCheckInDetails.companyEmail || "",
          companyPhone:
            existingCheckIn.corporateCheckInDetails.companyPhone || "",
          contactPersonName:
            existingCheckIn.corporateCheckInDetails.contactPersonName || "",
          designation:
            existingCheckIn.corporateCheckInDetails.designation || "",
          contactMobile:
            existingCheckIn.corporateCheckInDetails.contactMobile || "",
          contactEmail:
            existingCheckIn.corporateCheckInDetails.contactEmail || "",
  ...
  const [guests, setGuests] = useState<GuestEntry[]>([
    {
      ...
      name:
        ...
        "",
      mobileNo:
        bookingData?.bookingContact?.mobile ||
  ...
  const [dynamicDocs, setDynamicDocs] = useState<
    Array<{ id: string; type: string; file: File | null }>
  >([]);
  ...
  useEffect(() => {
    ...
    const slots: RoomEntry[] = bookingRooms.map((r: any, idx: number) => {
      ...
      const roomIdStr =
        roomIdObj?._id || (typeof r.roomId === "string" ? r.roomId : "");
      ...
      return {
        ...
        hasExtraBed: false,
        extraBedCharge:
          Number(roomIdObj?.extraBedCharge || r.extraBedCharge) || 0,
  ...
  const toggleRoom = (room: any) => {
    ...
    if (isSelected) {
      ...
      const removeIdx = selectedRooms.findIndex(
        (r) => r.roomId === roomIdToRemove,
      );
  ...
  const canProceed = (step: number): boolean => {
    switch (step) {
      case 1: {
        ...
        if (
          partyType === "Corporate" &&
          (!corporateDetails.companyName.trim() ||
            !corporateDetails.contactPersonName?.trim() ||
            !corporateDetails.contactMobile?.trim())
        ) {
          return false;
        }
        if (!isDayAccess) {
          ...
          if (
            ...
            assignedCount !== selectedRooms.length
          ) {
            return false;
          }
        }
        ...
        if (!isDayAccess) {
          const totalCapacity = selectedRooms.reduce(
            (sum, r) =>
              sum +
              (r.roomId
                ? (Number(r.maxAdults) || 2) +
                  (Number(r.maxChildren) || 0) +
                  (r.hasExtraBed ? 1 : 0)
                : 0),
            0,
          );
      ...
      case 1:
        ...
        if (!isDayAccess) {
          if (selectedRooms.length === 0)
            return "Please assign at least one room to continue.";
          ...
          if (unassignedCount > 0)
            return `Please assign all booked rooms to continue (${unassignedCount} left).`;
          ...
          if (totalCapacity > 0 && guests.length > totalCapacity)
            return "Total guests exceed the maximum capacity of assigned rooms.";
        }
        return "";
      case 2:
  ...
  const getStep2Validation = (): string => {
    ...
    if (primaryGuests.length === 0)
      return "At least one primary guest is required";

    for (let i = 0; i < primaryGuests.length; i++) {
      const primary = primaryGuests[i];
      if (!primary?.name?.trim())
        return `Primary guest name is required (Room ${i + 1})`;
      if (!primary?.mobileNo?.trim())
        return `Primary guest mobile is required (Room ${i + 1})`;
      if (!primary?.age?.trim())
        return `Primary guest age is required (Room ${i + 1})`;
      if (!primary?.gender)
        return `Primary guest gender is required (Room ${i + 1})`;
      if (!primary?.idNumber?.trim())
        return `Primary guest ID number is required (Room ${i + 1})`;
      if (!primary?.pendingDocFile && !primary?.idDocument)
        return `Primary guest ID document must be uploaded (Room ${i + 1})`;
    }
    ...
    try {
      ...
      for (let i = 0; i < primaryGuests.length; i++) {
        ...
        if (
          ...
          alert(
            `Primary guest name, mobile, age, gender, and ID number are required (Room ${i + 1})!`,
          );
          setLoading(false);
          return;
        }
      ...
      const payload = {
        ...
        primaryGuest: primaryGuests[0]
          ...
          : null,
        guests: guests
          .filter((g) => g.isPrimary || g.name?.trim())
          .map((g) => ({
            id: g.id,
            name: g.name,
            mobileNo: g.mobileNo,
      ...
      if (validDynamicDocs.length > 0) {
        multipartFormData.append(
          "dynamicDocTypes",
          JSON.stringify(validDynamicDocs.map((d) => d.type.trim())),
        );
      }
    ...
    return (
      ...
      >
        <div
          className={
            inline
              ? "w-full bg-white rounded-2xl p-10 border border-gray-100 shadow-xl text-center"
              : "max-w-md w-full bg-white rounded-2xl p-6 border border-gray-100 shadow-xl text-center"
          }
        >
          ...
          <p className="text-gray-500 text-sm mb-6 leading-relaxed">
            The scheduled check-in date for this booking is{" "}
            <strong className="text-gray-800">
              {new Date(bookingData.overallCheckInDate).toLocaleDateString(
                "en-GB",
                { day: "2-digit", month: "short", year: "numeric" },
              )}
            </strong>
            . Do you want to proceed with an early check-in?
          </p>
 ...
 `}</style>
      ...
      >
        {/* Header Stepper */}
        <div
          className={`px-4 sm:px-8 pt-4 pb-2 sm:pt-5 sm:pb-3 bg-white border-b border-border relative ${inline ? "rounded-t-2xl" : "rounded-t-2xl"}`}
        >
            ...
            <div className="flex justify-between relative z-10 w-full px-[2%] sm:px-[5%]">
              {[1, 2, 3].map((step) => {
                ...
                return (
                  <div
                    key={step}
                    className="flex flex-col items-center gap-2 sm:gap-3 w-1/3"
                  >
                    <div
            ...
            <div className="space-y-3 animate-fade-in">
                  ...
                  {partyType === "Corporate" && (
                    <div className="bg-white rounded-xl border border-border p-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                        <div>
                          <label className="text-[8px] 3xl:text-[12px] font-bold text-gray-400 uppercase block mb-1">
                            Company Name *
                          </label>
                          <input
                            type="text"
                        ...
                        <div>
                          ...
                          <input
                            type="tel"
                            value={corporateDetails.companyPhone}
                            onChange={(e) =>
                              setCorporateDetails({
                                ...corporateDetails,
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 3xl:grid-cols-[30%_70%] gap-3 mb-3">
                        <div>
                          <label className="text-[8px] 3xl:text-[12px] font-bold text-gray-400 uppercase block mb-1">
                            Company Email
                          </label>
                          <input
                            type="email"
                            value={corporateDetails.companyEmail}
                            onChange={(e) =>
                              setCorporateDetails({
                                ...corporateDetails,
                      ...
                      <div className="flex items-center gap-2 mt-4 mb-3 pt-3 border-t border-gray-100">
                        <h3 className="text-[10px] 3xl:text-[12px] font-black text-gray-700 uppercase tracking-wider">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                        ...
                        <div>
                          ...
                          <input
                            type="email"
                            value={corporateDetails.contactEmail}
                            onChange={(e) =>
                              setCorporateDetails({
                                ...corporateDetails,
                        ...
                        <div>
                          ...
                          <input
                            type="text"
                            value={corporateDetails.designation}
                            onChange={(e) =>
                              setCorporateDetails({
                                ...corporateDetails,
                  ...
                  {isDayAccess ? (
                    ...
                    <>
                      ...
                      <div className="bg-white rounded-xl border border-border p-4">
                        ...
                        <p className="text-[10px] 3xl:text-[14px] text-gray-400 mb-3">
                          Choose a pre-defined package for today's visit
                        </p>
                      ...
                      <div className="bg-white rounded-xl border border-border p-4">
                        ...
                        <div className="mb-4">
                          <label className="text-[9px] font-bold text-gray-400 uppercase block mb-2">
                            Add-ons & Facilities (Optional)
                          </label>
                          <div className="grid grid-cols-2 3xl:grid-cols-3 gap-2">
                            {extraServices.map((service: any) => {
                              const existing = selectedAddons.find(
                                (a: any) => a.serviceId === service._id,
                              );
                                ...
                                >
                                  <label className="flex items-center justify-between cursor-pointer w-full">
                                    <div className="flex items-center gap-2">
                                      ...
                                      />
                                      <div className="3xl:flex gap-2 items-center">
                                        <p className="font-bold text-[10px] 3xl:text-[14px] text-gray-800">
                                          {service.name}
                                        </p>
                                        <p className="text-[8px] 3xl:text-[12px] text-gray-400">
                                          ₹{service.price} /{" "}
                                          {service.chargeType || "Unit"}
                                        </p>
                                      </div>
                                    </div>
                                  </label>
                                  {isSelected && (
                                    <div className="mt-2 pt-2 border-t border-dashed border-orange-200 flex items-center justify-between">
                                      <span className="text-[9px] 3xl:text-[11px] text-gray-500 font-bold uppercase">
                                        Qty
                                      </span>
                                      <div className="flex items-center gap-2">
                                        <Counter
                                          ...
                                          max={50}
                                        />
                                        <span className="text-[10px] 3xl:text-[12px] font-bold text-gray-800 ml-1">
                      ...
                      <div className="bg-white rounded-xl border border-border p-4">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-4">
                          <div className="flex flex-col items-center justify-center p-2.5 bg-gray-50 border border-border rounded-xl">
                            <span className="text-[9px] 3xl:text-[12px] font-bold text-gray-400 uppercase mb-1.5">
                              Adults *
                            </span>
                              ...
                              <button
                                onClick={(e) => {
                                  ...
                                  if (
                                    ...
                                    removeGuest(
                                      adultGuests[adultGuests.length - 1].id,
                                    );
                                  }
                                }}
                              ...
                              <button
                                onClick={(e) => {
                                  ...
                                  if (
                                    ...
                                    removeGuest(
                                      childGuests[childGuests.length - 1].id,
                                    );
                                  }
                                }}
                                className="w-7 h-7 rounded-full bg-white border border-border flex items-center justify-center font-bold text-gray-600 hover:bg-gray-100 shadow-sm transition-colors"
                          ...
                          <div className="w-full flex-1 min-h-[58px] bg-gray-50 border border-border rounded-xl p-1.5 shadow-inner">
                            {selectedRooms.filter((r: any) => r.extraBedAllowed)
                              ...
                              <div className="text-[10px] 3xl:text-[12px] text-gray-400 text-center py-3 font-bold">
                                No rooms allow extra beds
                              </div>
                            ) : (
                              <div className="flex flex-col gap-1.5 px-1">
                                {selectedRooms
                                  .filter((r: any) => r.extraBedAllowed)
                                  .map((r: any) => (
                                    <label
                        ...
                        {/* Add-ons Checklist */}
                        <div className="mb-4">
                          <label className="text-[9px] 3xl:text-[12px] font-bold text-gray-400 uppercase block mb-2">
                            Add-ons & Facilities (Optional)
                          </label>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          ...
                          <div>
                            <label className="text-[8px] 3xl:text-[12px] font-bold text-gray-400 uppercase block mb-1">
                              Booking Reference / Remarks (Optional)
                            </label>
                            <input
                              type="text"
                  ...
                  {isDayAccess ? (
                    ...
                    <>
                      ...
                      <div className="bg-white rounded-xl border border-border p-4">
                        ...
                        <ul className="space-y-2 text-[10px] 3xl:text-[14px] text-gray-600 font-bold">
                          <li className="flex items-center gap-2 text-green-600">
                            ✅ Swimming Pool Access
                          </li>
                          <li className="flex items-center gap-2 text-green-600">
                            ✅ Locker Facility (1 Locker)
                          </li>
                          <li className="flex items-center gap-2 text-green-600">
                          ...
                          <li className="flex items-center gap-2 text-green-600">
                            ✅ Changing Room / Shower Access
                          </li>
                          <li className="flex items-center gap-2 text-green-600">
                            ✅ Safe Parking (One Vehicle)
                          </li>
                          <li className="flex items-center gap-2 text-green-600">
                            ✅ Wi-Fi Access in Common Areas
                          </li>
                          <li className="flex items-center gap-2 text-green-600">
                            ✅ Valid from 10:00 AM to 06:00 PM
                          </li>
                        </ul>
                      </div>
                      ...
                      <div className="bg-white rounded-xl border border-border p-4 space-y-3">
                        ...
                        <div className="space-y-2 text-[11px] 3xl:text-[14px] font-bold text-gray-600">
                          ...
                          <div className="flex justify-between text-[10px] 3xl:text-[14px] text-gray-400">
                          ...
                          {selectedAddons
                            .filter((a: any) => a.taxAmount > 0)
                            .map((addon: any, idx) => (
                              <div
                                key={`addon-tax-${idx}`}
                                className="flex justify-between text-[10px] text-gray-400 pl-2"
                              >
                                <span>
                        ...
                        <div className="p-2.5 bg-blue-50 text-blue-600 border border-blue-100 rounded-lg text-[9px] 3xl:text-[14px] font-bold">
                          ℹ️ Actual amount may change slightly in final step
                          based on offers / taxes.
                        </div>
                      </div>
                    </>
                  ) : (
                    ...
                    <>
                      ...
                      <div className="bg-white rounded-xl border border-border p-4 flex flex-col">
                        ...
                        {(() => {
                          const bookedSummary = bookingData?.rooms?.reduce(
                            (acc: any, room: any) => {
                              const name =
                                room.roomTypeName ||
                                room.roomType?.name ||
                          ...
                          const bookedSummaryText = bookedSummary
                            ? Object.entries(bookedSummary)
                                .map(([k, v]) => `${k} - ${v}`)
                                .join(", ")
                            : "None";

                          const selectedSummary = selectedRooms
                            .filter((r) => r.roomId)
                        ...
                        {selectedRooms.length > 0 && (
                          <div className="flex flex-wrap gap-2 p-2 bg-orange-50/50 border border-orange-100 rounded-xl mb-3">
                            {selectedRooms.map((room) => {
                              ...
                              let typeName =
                                room.roomTypeName || "Any Room Type";
                              if (isUnassigned && room.requiredRoomTypeId) {
                                const origType = roomTypes.find(
                                  (t: any) => t._id === room.requiredRoomTypeId,
                                );
                              ...
                              return (
                                <div
                                  ...
                                  title={
                                    isUnassigned && room.requiredRoomTypeId
                                      ? "Click to filter table by this category"
                                      : ""
                                  }
                                >
                                  <span className="text-gray-500 uppercase tracking-widest text-xs mr-1 border-r border-orange-200 pr-1.5">
                                    {typeName}
                                  </span>
                                  <span
                                    className={
                                  ...
                                  >
                                    {room.roomNumber || "Select Room"}
                                  </span>
                                  {!isUnassigned && (
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        toggleRoom({ _id: room.roomId });
                                      }}
                                      className="hover:text-red-500 font-bold ml-1.5 text-lg leading-none transition-colors"
                                    >
                                      ×
                                    </button>
                                  )}
                            ...
                            <tbody>
                              {loadingRooms ? (
                                <tr>
                                  ...
                                  >
                                    <FiLoader
                                      className="animate-spin mx-auto mb-2 text-orange-500"
                                      size={24}
                                    />
                                    <p>Loading rooms...</p>
                                  </td>
                        ...
                        {(() => {
                          ...
                          if (totalCapacity === 0) {
                            return (
                              <div className="mt-3 p-2.5 bg-gray-50 border border-gray-200 rounded-xl text-gray-500 text-sm font-bold flex items-center justify-between">
                                <span>
                                  🕒 Occupancy Validation: Pending Room
                                  Assignment
                                </span>
                                <span className="px-1.5 py-1 bg-gray-200 text-gray-600 rounded text-xs uppercase font-black">
                                  Pending
                                </span>
                              </div>
                            );
                          ...
                          return (
                            <div
                              className={`mt-3 p-2.5 border rounded-xl text-sm font-bold flex items-center justify-between ${isValid ? "bg-green-50 border-green-200 text-green-800" : "bg-red-50 border-red-200 text-red-800"}`}
                            >
                              <span>
                                {isValid ? "💚" : "⚠️"} Occupancy Validation:{" "}
                                {isValid ? "Valid" : "Exceeds Capacity"}{" "}
                                (Guests: {guests.length} / Capacity:{" "}
                                {totalCapacity})
                              </span>
                              <span
                                className={`px-1.5 py-1 rounded text-xs uppercase font-black ${isValid ? "bg-green-200 text-green-700" : "bg-red-200 text-red-700"}`}
                              >
              ...
              {!isDayAccess && (
                <>
                  ...
                  <div className="bg-white rounded-xl border border-border p-4">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-6 gap-4 text-sm pt-3 border-t border-gray-100">
                      ...
                      <div>
                        ...
                        <p className="font-bold 3xl:text-[12px] text-gray-800 text-base">
                          {selectedRooms.length * 2} Guests
                        </p>
                        <p className="text-[8px] 3xl:text-[12px] text-gray-400">
                          From Selected Rooms
                        </p>
                      ...
                      <div>
                        ...
                        <p className="text-[8px] 3xl:text-[12px] text-gray-400">
                          ({format(stayFormData.checkInTime, "dd MMM")} -{" "}
                          {format(stayFormData.expectedCheckOutTime, "dd MMM")})
                        </p>
                      </div>
                      <div>
                        <p className="text-[9px] 3xl:text-[12px] text-gray-400 uppercase font-black mb-1">
                      ...
                      <div>
                        ...
                        <p className="text-[8px] 3xl:text-[12px] text-orange-500 font-bold hover:underline cursor-pointer">
                          Change Tariff
                        </p>
                      </div>
                    </div>
                  ...
                  {/* Room Holding Banner */}
                  <div className="bg-amber-50 border border-amber-200 text-amber-800 rounded-xl p-3 text-[10px] 3xl:text-[14px] font-bold flex items-center gap-2">
                    <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse"></span>
                    Room(s) will be held for 10 minutes while you complete the
                    check-in.
                  </div>
                </>
              )}
            ...
            <div className="space-y-3 animate-fade-in">
              <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
                ...
                <div className="lg:col-span-3 space-y-3">
                  ...
                  {partyType === "Corporate" ||
                  selectedRooms.filter((r) => !!r.roomId).length === 0 ? (
                    <div className="bg-white rounded-xl border border-border p-4 mb-3">
                      ...
                      {(() => {
                        const primaryGuest =
                          guests.find((g) => g.isPrimary) || guests[0];
                        const coGuests = guests.filter(
                          (g) => g.id !== primaryGuest?.id,
                        );
                              ...
                              <div>
                                ...
                                <input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    if (/^[0-9]*$/.test(val))
                                      updateGuest(primaryGuest.id, "age", val);
                                  }}
                                  placeholder="e.g. 32"
                                  className="w-full p-2 bg-gray-50 border border-border rounded-lg text-sm font-bold outline-none"
                                />
                              ...
                              <div>
                                ...
                                {primaryGuest.pendingDocFile ? (
                                  <div className="flex items-center justify-between p-2 bg-green-50 border border-green-200 rounded-lg text-[10px] font-bold">
                                    ...
                                    <button
                                      onClick={() =>
                                        handleRemoveDocument(primaryGuest.id)
                                      }
                              ...
                              {coGuests.length > 0 && (
                                <table className="w-full text-left text-[10px]">
                                  <thead className="bg-gray-50 border-b border-gray-200 text-gray-500">
                                    <tr>
                                      <th className="p-2 font-black uppercase tracking-wider">
                                        Full Name
                                      </th>
                                      <th className="p-2 font-black uppercase tracking-wider w-16">
                                        Age
                                      </th>
                                      <th className="p-2 font-black uppercase tracking-wider w-24">
                                        Gender
                                      </th>
                                      <th className="p-2 font-black uppercase tracking-wider w-32">
                                        ID Proof
                                      </th>
                                      <th className="p-2 font-black uppercase tracking-wider w-12 text-center">
                                        Action
                                      </th>
                                    </tr>
                                  </thead>
                                      ...
                                      >
                                        ...
                                        <td className="p-2">
                                          <select
                                            ...
                                            className="w-full bg-transparent outline-none font-bold text-gray-700 cursor-pointer"
                                          >
                                            <option value="">Select</option>
                                            <option value="Male">Male</option>
                                            <option value="Female">
                                        ...
                                        <td className="p-2">
                                          <div className="flex flex-col gap-1">
                                            <select
                                              ...
                                              className="w-full bg-transparent outline-none font-bold text-gray-500 cursor-pointer text-[9px]"
                                            >
                                            ...
                                            {g.idType !== "Not Required" &&
                                              (g.pendingDocFile ? (
                                                <div className="flex items-center justify-between p-1 bg-green-50 border border-green-200 rounded text-[8px] font-bold mt-1">
                                                  ...
                                                  <button
                                                    onClick={() =>
                                                      handleRemoveDocument(g.id)
                                                    }
                                                    className="text-red-500 hover:text-red-700 ml-1 font-black text-xs"
                                                  >
                                                    ×
                                                  </button>
                                                </div>
                                        ...
                                        <td className="p-2 text-center">
                                          <button
                                            ...
                                            className="text-red-400 hover:text-red-600 p-1 opacity-50 group-hover:opacity-100 transition-opacity"
                                            title="Remove Guest"
                                          >
                                            🗑️
                                          </button>
                    ...
                    selectedRooms
                      ...
                      .map((room, roomIdx) => {
                        const roomGuests = guests.filter(
                          (g) => g.assignedRoomId === room.roomId,
                        );
                        const primaryGuest =
                          roomGuests.find((g) => g.isPrimary) || roomGuests[0];
                        const coGuests = roomGuests.filter(
                          (g) => g.id !== primaryGuest?.id,
                        );
                            ...
                            <div
                              ...
                              onClick={() =>
                                setExpandedRooms((prev) => ({
                                  ...prev,
                                  [room.roomId]:
                                    prev[room.roomId] === false ? true : false,
                                }))
                              }
                            >
                              <div className="flex items-center gap-2">
                                      ...
                                      {roomIdx === 0 && (
                                        <div>
                                          ...
                                          >
                                            ...
                                            <option value="Medical Tourism">
                                              Medical Tourism
                                            </option>
                                            <option value="Personal Stay">
                                              Personal Stay
                                            </option>
                                          </select>
                                        </div>
                    ...
                    <div className="">
                      ...
                      <div className="space-y-3">
                        {dynamicDocs.map((doc, idx) => (
                          <div
                            key={doc.id}
                            className="flex flex-col sm:flex-row gap-3 items-start sm:items-center bg-gray-50 p-2 border border-gray-200 rounded-lg"
                          >
                            ...
                            <button
                              onClick={() =>
                                setDynamicDocs(
                                  dynamicDocs.filter((d) => d.id !== doc.id),
                                )
                              }
                      ...
                      <button
                        onClick={() =>
                          setDynamicDocs([
                            ...dynamicDocs,
                            { id: `doc-${Date.now()}`, type: "", file: null },
                          ])
                        }
                  ...
                  <div className="bg-white rounded-xl border border-border p-4">
                    ...
                    <div className="space-y-3">
                      {vehicles.map((v, index) => (
                        <div
                          key={index}
                          className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end p-3 bg-gray-50 border border-border rounded-xl relative"
                        >
                          {vehicles.length > 1 && (
                            <button
                    ...
                    <div className="grid grid-cols-3 gap-1.5">
                      {paymentModes.map((mode) => (
                        ...
                        >
                          ...
                          />
                          <span className="font-bold text-[9px] text-gray-800">
                            {mode.label}
                          </span>
                        </button>
                      ))}
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (/^\d*$/.test(val)) {
                            setPaymentData({
                              ...paymentData,
                              checkInAdvance: val === "" ? 0 : Number(val),
                            });
                          }
                        }}
                        placeholder="₹16,000"
                        ...
                        {selectedRooms.map((room) => (
                          ...
                          >
                            ...
                            <span>
                              ₹
                              {(
                                (Number(room.basePrice) || 0) * nights
                              ).toLocaleString()}
                            </span>
                          </div>
                        ))}
                        {selectedRooms.some((r) => r.hasExtraBed) && (
                          <div className="flex justify-between text-orange-500">
                            ...
                            <span>
                              ...
                              {(
                                selectedRooms
                                  .filter((r) => r.hasExtraBed)
                                  .reduce(
                                    (sum, r) =>
                                      sum + (Number(r.extraBedCharge) || 0),
                                    0,
                                  ) * nights
                              ).toLocaleString()}
                            </span>
                  ...
                  <div className="space-y-3">
                      ...
                      <div className="border-t border-gray-200 pt-2.5">
                        ...
                        {signedGRCFile || signedGRCPreview ? (
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              e.target.files?.[0] &&
                              handleSignedGRCUpload(e.target.files[0])
                            }
                            className="w-full text-sm text-gray-400 font-bold"
                          />
                        )}
                        ...
                        <span className="text-[8px] text-green-600 font-black uppercase">
                          Uploaded (
                          {
                            guests.filter(
                              (g) =>
                                g.idType !== "Not Required" &&
                                (g.idDocument || g.pendingDocFile),
                            ).length
                          }{" "}
                          /{" "}
                          {
                            guests.filter((g) => g.idType !== "Not Required")
                              .length
                          }
                          )
                        </span>
                    ...
                    <div className="p-3 bg-gray-50 border border-border rounded-xl">
                      ...
                      <ul className="space-y-1 text-[10px] font-bold text-gray-600">
                        ...
                        <li className="flex items-center gap-1.5 text-green-600">
                          {guests[0]?.pendingDocFile || guests[0]?.idDocument
                            ? "✅"
                            : "❌"}{" "}
                          Primary ID Proof
                        </li>
                        <li className="flex items-center gap-1.5 text-green-600">
                          {guests.every(
                            ...
                            ? "✅"
                            : "❌"}{" "}
                          Co-Guest IDs (Optional/Uploaded)
                        </li>
                        <li className="flex items-center gap-1.5 text-green-600">
                          {paymentData.checkInAdvance >= dueAmount
                            ? "✅"
                            : "ℹ️"}{" "}
                          Payment Collected
                        </li>
