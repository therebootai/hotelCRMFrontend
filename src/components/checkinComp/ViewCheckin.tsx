      ...
      <div className="bg-white w-full max-w-4xl rounded-xl shadow-2xl max-h-[95vh] flex flex-col">
        ...
        <div className="px-4 py-3 border-b border-gray-100 flex justify-between items-center bg-gray-50/50 rounded-t-xl">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-black text-gray-800 uppercase">
                Check-in Details
              </h2>
              <p className="text-[10px] text-gray-500 font-bold">
                {checkIn.checkInId} •{" "}
                {checkIn.bookingId?.bookingId || checkIn.bookingId}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          ...
          <div className="flex items-center gap-2 text-[10px]">
            <span
              className={`px-2 py-0.5 rounded-full font-bold uppercase ${checkIn.status === "Active" ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-500"}`}
            >
              {checkIn.status}
            </span>
            <span
              className={`px-2 py-0.5 rounded-full font-bold uppercase ${checkIn.paymentStatus === "Paid" ? "bg-green-100 text-green-600" : checkIn.paymentStatus === "Partial" ? "bg-yellow-100 text-yellow-600" : "bg-red-100 text-red-600"}`}
            >
              ...
              <div className="space-y-1 text-[10px]">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-gray-800">
                    {primaryGuest?.name || "N/A"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">Mobile</span>
                  <span className="font-bold text-gray-800">
                    {primaryGuest?.mobileNo || "N/A"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">ID Type</span>
                  <span className="font-bold text-gray-800">
                    {primaryGuest?.idType || "N/A"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">ID Number</span>
                  <span className="font-bold text-gray-800">
                    {primaryGuest?.idNumber || "N/A"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">Nationality</span>
                  <span className="font-bold text-gray-800">
                    {primaryGuest?.nationality || "Indian"}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-3 border border-gray-100">
              ...
              <div className="space-y-1 text-[10px]">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-gray-800">
                    {checkIn.checkInTime
                      ? format(new Date(checkIn.checkInTime), "dd MMM yy HH:mm")
                      : "N/A"}
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-gray-800">
                    {checkIn.bookingCategory || "Room Stay"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">Stay Type</span>
                  <span className="font-bold text-gray-800">
                    {checkIn.stayType || "Original"}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-gray-50 rounded-lg p-3 border border-gray-100">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[10px] font-black text-gray-500 uppercase">
                Rooms ({checkIn.roomDetails?.length || 0})
              </span>
            </div>
              ...
              {checkIn.roomDetails?.map((room: any, idx: number) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    <span className="w-8 h-8 bg-orange-100 text-orange-600 rounded-lg flex items-center justify-center font-black text-[10px]">
                      {room.roomNumber || "TBD"}
                    </span>
                    <div>
                      <span className="font-bold text-gray-800">
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-white rounded p-2 text-center border border-green-100">
                ...
                <span className="text-base font-black text-gray-800">
                  ₹{checkIn.paymentSummary?.totalAmount?.toLocaleString() || 0}
                </span>
              </div>
              <div className="bg-white rounded p-2 text-center border border-green-100">
                <span className="text-[9px] text-gray-400 block">Paid</span>
                <span className="text-base font-black text-green-600">
                  ₹{checkIn.paymentSummary?.totalPaid?.toLocaleString() || 0}
                </span>
              </div>
              <div className="bg-white rounded p-2 text-center border border-green-100">
                <span className="text-[9px] text-gray-400 block">Due</span>
                <span className="text-base font-black text-red-500">
                  ₹{checkIn.paymentSummary?.dueAmount?.toLocaleString() || 0}
                </span>
              </div>
            </div>
            {checkIn.payments?.length > 0 && (
              <div className="mt-2 space-y-1">
                {checkIn.payments.map((payment: any, idx: number) => (
                  <div
                    key={idx}
                    className="flex justify-between text-[10px] p-1 bg-white rounded border border-green-100"
                  >
                    <span className="text-gray-500">
                      {payment.paymentMode}: ₹{payment.amount?.toLocaleString()}
                    </span>
                    <span className="text-gray-400">
                      {payment.paidAt
                        ? format(new Date(payment.paidAt), "dd MMM")
                        : ""}
                    </span>
                  </div>
                ))}
          ...
          {/* Vehicle Details */}
          {checkIn.vehicleDetails?.length > 0 &&
            checkIn.vehicleDetails.some((v: any) => v.vehicleNumber) && (
              <div className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                <div className="flex items-center gap-2 mb-2">
            ...
            <div className="bg-purple-50 rounded-lg p-3 border border-purple-100">
              ...
              <div className="space-y-1">
                {checkIn.grcDetails.map((grc: any, idx: number) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between p-2 bg-white rounded border border-purple-100 text-[10px]"
                  >
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-gray-800">
                        {grc.grcNumber}
                      </span>
                      <span className="px-1 py-0.5 bg-purple-100 text-purple-600 rounded text-[8px] font-bold">
                        {grc.grcType}
                      </span>
                    </div>
                    <span
          ...
          {checkIn.checkInType === "Corporate" &&
            checkIn.corporateCheckInDetails && (
              <div className="bg-blue-50 rounded-lg p-3 border border-blue-100">
                ...
                <div className="grid grid-cols-2 gap-2 text-[10px]">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-gray-800">
                      {checkIn.corporateCheckInDetails.companyName || "N/A"}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400">GST</span>
                    <span className="font-bold text-gray-800">
                      {checkIn.corporateCheckInDetails.companyGST || "N/A"}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400">Contact</span>
                    <span className="font-bold text-gray-800">
                      {checkIn.corporateCheckInDetails.contactPersonName ||
                        "N/A"}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400">Mobile</span>
                    <span className="font-bold text-gray-800">
                      {checkIn.corporateCheckInDetails.contactMobile || "N/A"}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="px-4 py-3 border-t bg-gray-50/50 rounded-b-xl flex justify-between items-center">
          <span className="text-[9px] text-gray-400 font-bold uppercase">
            Created:{" "}
            {checkIn.createdAt
              ? format(new Date(checkIn.createdAt), "dd MMM yy HH:mm")
              : ""}
          </span>
          <button
            onClick={onClose}
            className="px-4 py-2 bg-gray-100 text-gray-600 rounded-lg font-bold text-[10px] uppercase hover:bg-gray-200 transition-all"
          >
            Close
          </button>
        </div>
