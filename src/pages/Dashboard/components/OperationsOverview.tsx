...
const ExpectedArrivalsCard = ({
  ...
  <div className="card flex flex-col h-full">
    ...
    <div className="flex justify-between items-center mb-4">
      ...
      <span className="text-sm font-bold bg-primary/10 text-primary px-2.5 py-1 rounded-md tracking-wide">
        {arrivalsCount} Today
      </span>
    </div>
      ...
      {(arrivals.length > 0 ? arrivals : []).map((guest) => (
        ...
        >
          <div className="flex items-center gap-3">
            {/* Avatar */}
            <div
              className={`w-10 h-10 rounded-xl flex items-center justify-center text-base font-bold ${guest.avatarBg} ${guest.avatarColor}`}
            >
              {guest.initials}
            </div>
            {/* Details */}
...
const OccupiedRoomsCard = ({
  ...
  <div className="card flex flex-col h-full">
    {/* Header */}
    <div className="flex justify-between items-center mb-4">
      <h3 className="text-base 3xl:text-[16px] font-semibold text-text-primary">
        Occupied Rooms
      </h3>
      ...
      {(floorOccupancy.length > 0 ? floorOccupancy : []).map((floor) => (
        <div key={floor.id} className="flex flex-col gap-2">
          <div className="flex justify-between items-end">
            <span className="text-[10px] 3xl:text-[14px] font-bold text-gray-600 uppercase">
              {floor.label}
            </span>
            <span className="text-[8px] 3xl:text-[13px] font-bold text-text-primary">
  ...
  <div className="card flex flex-col h-full">
    ...
    <div className="flex flex-col gap-3 flex-1">
      {(departures.length > 0 ? departures : []).map((guest) => (
        <div
          key={guest.id}
          className="flex items-center justify-between bg-gray-100 rounded-xl p-3 relative overflow-hidden"
        >
          {/* Urgent Red Marker */}
          {guest.isUrgent && (
          ...
          <div className={`flex flex-col ${guest.isUrgent ? "pl-2" : ""}`}>
            ...
            <span
              className={`text-[10px] font-bold tracking-wider ${guest.isUrgent ? "text-primary" : "text-text-secondary"}`}
            >
              {guest.time}
            </span>
          </div>
