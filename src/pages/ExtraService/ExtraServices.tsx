...
export default function ExtraServiceMaster() {
 ...
 setExtraServices(prev => prev.map(s => s._id === id ? { ...s, isActive: !currentStatus } : s));
 ...
 setExtraServices(prev => prev.map(s => s._id === id ? { ...s, isActive: updatedService.isActive } : s));
 }
 ...
 return (
 <div className="flex flex-col w-full h-full p-8 max-w-300 mx-auto relative page-container animate-fade-in">
 
 {/* Page Header (Added for standalone page context) */}
 <div className="flex justify-between items-center mb-8">
 <div>
 <h1 className="text-2xl font-bold text-text-primary">Extra Services</h1>
 <p className="text-base text-text-secondary mt-1">
 Manage additional guest offerings like laundry, ironing, and transport.
 </p>
 </div>
 <button
 onClick={() => {
 ...
 >
 <span className={`absolute top-0.5 left-0.5 bg-card w-4 h-4 rounded-full transition-transform duration-200 ${service.isActive ? 'translate-x-5' : 'translate-x-0'}`} />
 </button>
 </td>
 <td className="px-6 py-4">
