  ...
  const handleToggleStatus = async (id: string, currentStatus: boolean) => {
    ...
    try {
      ...
      if (updatedAmenity && typeof updatedAmenity.isActive === "boolean") {
        setAmenities((prev) =>
          prev.map((a) =>
            a._id === id ? { ...a, isActive: updatedAmenity.isActive } : a,
          ),
        );
      }
        ...
        <div className="overflow-x-auto no-scrollbar">
          <table className="w-full text-left border-collapse">
            <thead className="bg-background/50 border-b border-border">
              <tr>
                <th className="px-10 py-4 text-[11px] 3xl:text-[14px] 4xl:text-[16px] font-semibold text-text-secondary uppercase tracking-wider w-24 text-center">
                  Icon Preview
                </th>
                <th className="px-10 py-4 text-[11px] 3xl:text-[14px] 4xl:text-[16px] font-semibold text-text-secondary uppercase tracking-wider w-[30%]">
                  Amenity Name
                </th>
                <th className="px-10 py-4 text-[11px] 3xl:text-[14px] 4xl:text-[16px] font-semibold text-text-secondary uppercase tracking-wider">
                  Status
                </th>
                <th className="px-10 py-4 text-[11px] 3xl:text-[14px] 4xl:text-[16px] font-semibold text-text-secondary uppercase tracking-wider text-right w-32">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {isLoading ? (
                ...
                amenities.map((amenity) => {
                  const IconComponent =
                    ICON_MAP[amenity.icon] || ICON_MAP["wifi"];

                  return (
                    <tr
                      key={amenity._id}
                      className={`hover:bg-background/50 transition-colors group ${!amenity.isActive ? "opacity-60" : ""}`}
                    >
                      ...
                      <td className="px-10 py-4 text-left">
                        <button
                          onClick={() =>
                            handleToggleStatus(amenity._id, amenity.isActive)
                          }
                          className={`w-10 h-5 rounded-full relative transition-colors duration-200 ${amenity.isActive ? "bg-primary" : "bg-border"}`}
                        >
                          <span
                            className={`absolute top-0.5 left-0.5 bg-card w-4 h-4 rounded-full transition-transform duration-200 ${amenity.isActive ? "translate-x-5" : "translate-x-0"}`}
                          />
                        </button>
                      </td>
                      <td className="px-10 py-4">
