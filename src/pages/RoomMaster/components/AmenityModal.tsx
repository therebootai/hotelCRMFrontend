...
export default function AmenityModal({ isOpen, onClose, onSuccess, initialData }: AmenityModalProps) {
 ...
 <span className="text-text-primary text-base font-medium capitalize">{formData.icon}</span>
 </div>
 </button>
 {errors.icon && (
 <p className="text-red-500 text-sm mt-1.5 font-medium animate-fade-in">{errors.icon}</p>
 )}
