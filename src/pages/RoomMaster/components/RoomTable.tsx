...
export default function RoomTable({
  ...
  const isAllCurrentPageSelected =
    ...
    currentPageIds.every((id) => selectedRoomIds.includes(id));
  const isSomeCurrentPageSelected = currentPageIds.some((id) =>
    selectedRoomIds.includes(id),
  );

  const cellPadding = isSelectionMode ? "px-3 py-2" : "px-10 py-4";
  const headerPadding = isSelectionMode
    ? "px-3 py-3 text-[10px]"
    : "px-10 py-4 text-[11px]";
  const textScale = isSelectionMode ? "text-sm" : "text-base";

  const handleSelectAll = () => {
    if (isAllCurrentPageSelected) {
      onSelectionChange(
        selectedRoomIds.filter((id) => !currentPageIds.includes(id)),
      );
    } else {
  ...
  const handleSelectRow = (id: string) => {
    if (selectedRoomIds.includes(id)) {
      onSelectionChange(
        selectedRoomIds.filter((selectedId) => selectedId !== id),
      );
    } else {
  ...
  const handleStatusChange = async (
    ...
    try {
      ...
      if (updatedRoom && updatedRoom.status) {
        setTableData((prev) =>
          prev.map((r) =>
            r._id === id ? { ...r, status: updatedRoom.status } : r,
          ),
        );
      }
      ...
      setTableData((prev) =>
        prev.map((r) => (r._id === id ? { ...r, status: oldStatus } : r)),
      );
        ...
        <table className="w-full text-left border-collapse whitespace-nowrap">
          <thead className="bg-background/80 sticky top-0 z-10 border-b border-border backdrop-blur-sm">
            <tr>
              ...
              {!isSelectionMode && (
                <th
                  className={`${headerPadding} font-semibold 3xl:text-[14px] text-text-secondary uppercase tracking-wider text-right`}
                >
                  Actions
                </th>
              )}
              ...
              tableData.map((room) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td
                      className={`${cellPadding} font-medium text-text-primary ${textScale}`}
                    >
                      {room.roomNumber}
                    </td>
                    <td
                      className={`${cellPadding} text-text-secondary ${textScale}`}
                    >
                      {room.roomType?.name || "---"}
                    </td>
                    <td
                    ...
                    <td className={cellPadding}>
                      <div className="flex items-center gap-2 bg-background border border-border rounded-lg px-2 py-1 w-fit">
                        ...
                        >
                          ...
                          <option value="Blocked" className="text-text-primary">
                            Blocked
                          </option>
                        </select>
                      </div>
      ...
      <div className="px-4 py-3 border-t border-border flex items-center justify-between bg-card rounded-b-xl">
        <div className={`text-text-secondary ${textScale}`}>
          ...
          <span className="font-medium text-text-primary">
            {tableData.length > 0 ? startIndex + 1 : 0}
          </span>{" "}
          to <span className="font-medium text-text-primary">{endIndex}</span>{" "}
