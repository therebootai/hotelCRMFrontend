...
export default function TaxGstMaster({
  isAddModalOpen,
  setIsAddModalOpen,
}: TaxGstMasterProps) {
  ...
  const handleToggleStatus = async (id: string, currentStatus: boolean) => {
    ...
    try {
      ...
      if (updatedTax && typeof updatedTax.isActive === "boolean") {
        setTaxes((prev) =>
          prev.map((t) =>
            t._id === id ? { ...t, isActive: updatedTax.isActive } : t,
          ),
        );
      }
      toast.success(
        `Tax rule ${!currentStatus ? "activated" : "deactivated"}.`,
      );
    } catch (error: unknown) {
      setTaxes((prev) =>
        prev.map((t) => (t._id === id ? { ...t, isActive: currentStatus } : t)),
  ...
  return (
    <div className="mt-4 animate-fade-in space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-4 text-base">
          <div className="flex items-center gap-3">
            ...
            <select
              ...
              disabled={isLoading}
              className="input-field py-1.5 px-3 min-w-30 bg-background border-border text-base disabled:opacity-70 cursor-pointer"
            >
            ...
            <div className="flex bg-background border border-border rounded-lg p-1">
              {["All", "Room", "Food", "Service"].map((filter) => (
                <button
                  ...
                  disabled={isLoading}
                  onClick={() =>
                    setTypeFilter(filter as "Room" | "Food" | "Service" | "All")
                  }
                  className={`px-4 py-1.5 rounded-md font-medium transition-colors text-sm sm:text-base disabled:opacity-70 ${
                    typeFilter === filter
      ...
      <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
        <div className="overflow-x-auto no-scrollbar">
          <table className="w-full text-left border-collapse">
            <thead className="bg-background/50 border-b border-border">
              <tr>
                <th className="px-10 py-4 text-[11px] 3xl:text-[14px] font-semibold text-text-secondary uppercase tracking-wider w-[35%]">
                  Tax Name
                </th>
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? (
                <tr>
                  ...
                  >
                    <div className="flex flex-col items-center justify-center gap-2">
                      ...
                      <span className="text-base">Loading taxes...</span>
                    </div>
                  </td>
                </tr>
                ...
                filteredTaxes.map((tax) => (
                  <tr
                    key={tax._id}
                    className={`hover:bg-background/50 transition-colors group ${!tax.isActive ? "opacity-60" : ""}`}
                  >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-sm text-text-secondary mt-0.5">
                            {tax.type === "Room"
                              ? "Applied to stay tariffs"
                              : tax.type === "Food"
                                ? "Applied to F&B bills"
                    ...
                    <td className="px-10 py-5">
                      <span className="text-lg font-bold text-text-primary">
                        ...
                        <span className="text-base text-text-secondary ml-0.5">
                          %
                        </span>
                      </span>
                    </td>
                    ...
                    <td className="px-10 py-5">
                      <button
                        onClick={() =>
                          handleToggleStatus(tax._id, tax.isActive)
                        }
                        className={`w-10 h-5 rounded-full relative transition-colors duration-200 ${tax.isActive ? "bg-primary" : "bg-border"}`}
                      >
                        <span
                          className={`absolute top-0.5 left-0.5 bg-card w-4 h-4 rounded-full transition-transform duration-200 ${tax.isActive ? "translate-x-5" : "translate-x-0"}`}
                        />
                      </button>
                    </td>
                    <td className="px-10 py-5">
        ...
        {!isLoading && (
          <div className="px-10 py-4 border-t border-border bg-background flex items-center justify-between text-base text-text-secondary">
            <span>
              Showing {filteredTaxes.length} of {taxes.length} tax
              configurations
            </span>
          </div>
        )}
