...
const BillingPage = () => {
  ...
  const fetchBillings = useCallback(async () => {
    try {
      ...
      console.error(err);
      toast.error(
        err.response?.data?.message || "Failed to fetch billing list",
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex flex-wrap items-center justify-between gap-3">
        <div className="relative flex-1 min-w-[280px]">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-transparent focus:border-orange-200 focus:bg-white rounded-xl outline-none transition-all text-sm"
            value={search}
            onChange={(e) =>
              setMultipleParams(
